...
void cashFlowMenu(BankCustomer& account); // cashFlowMenu dideklarasikan
...
int main() {
    ...
    try {
        BankCustomer& aliceAcc = myBank.createAccount("Alice", 2000.0, "Jl. Utama", "08123", "alice@test.com"); // LN 77-79
        ...
        BankCustomer& bobAcc = myBank.createAccount("Bob", 500.0, "Jl. Kedua", "08987", "bob@test.com"); // LN 82
        buyers.emplace_back(++buyerIdCounter, "Bob", bobAcc);
        
        BankCustomer& charlieAcc = myBank.createAccount("Charlie", 1000.0, "Jl. Ketiga", "08888", "charlie@test.com"); // LN 85
...
void buyerMenu() {
    ...
    do {
        ...
        if (choice == 1) { 
            ...
            cashFlowMenu(currentBuyer->getAccount()); // LN 305
        }
...
void upgradeToSeller() {
    ...
    cout << "Your account is linked with Bank ID: " << currentBuyer->getAccount().getId() << endl; // LN 390
...
void paymentFunctionality() {
    ...
    cout << "Your Balance: $" << currentBuyer->getAccount().getBalance() << endl; // LN 542
    cout << "Processing payment..." << endl;

    if (!myBank.withdraw(currentBuyer->getAccount().getId(), total)) { // LN 545
...
void checkSpendingLastKDays(int k) {
    ...
    for (const auto& flow : cashFlows) {
        if (flow.getAccountId() == currentBuyer->getAccount().getId() && // LN 570
...
void cashFlowMenu(BankCustomer& account) { // LN 604
    int choice;
    double amount;
    do {
